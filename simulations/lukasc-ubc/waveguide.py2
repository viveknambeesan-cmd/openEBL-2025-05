...
class Waveguide():
    ...
    def __init__(self):
        ...
        self.wavelengths = np.arange(self.wavelength_start, self.wavelength_stop, self.wavelength_points)
    ...
    def simulate(self):
        ...
        for i in range(self.wavelengths):
            mode.switchtolayout()
            mode.select('MODE')
            mode.setnamed('MODE', 'wavelength', self.wavelengths[i])
            mode.run()
            mode.mesh()
            ...
            mode.setanalysis('wavelength', self.wavelengths[i])
    ...
    def save(self):
        ...
        with open(output_file, 'a') as file:
            for i in range(self.wavelengths):
                file.write(f'{self.wg_width}, {self.wg_height}, {self.wg_material_core}, {self.wg_material_clad}, {self.wavelengths[i]}, {self.neff1[i]}\n')
